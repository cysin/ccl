...
__global__ void kernel(int D[], int L[], bool* m, int N, int W, int th)
{
	int id = blockIdx.x * blockDim.x + blockIdx.y * blockDim.x * gridDim.x + threadIdx.x;
	if (id >= N) return;
...
__global__ void kernel8(int D[], int L[], bool* m, int N, int W, int th)
{
	int id = blockIdx.x * blockDim.x + blockIdx.y * blockDim.x * gridDim.x + threadIdx.x;
	if (id >= N) return;
...
vector<int> CCL::cuda_ccl(vector<int>& image, int W, int degree_of_connectivity, int threshold)
{
	...
	for (;;) {
		...
		if (degree_of_connectivity == 4) kernel<<<grid, threads>>>(Dd, Ld, md, N, W, threshold);
		else kernel8<<<grid, threads>>>(Dd, Ld,md, N, W, threshold);
